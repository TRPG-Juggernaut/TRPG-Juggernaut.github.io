...
const defaultUsuarios = [
    ...
    { id: 2, nombre: "Jose Julian Tello Diaz", correo: "tello.cisco@itsr.edu.mx", password: "cisco123", rol: "admin" },
    { id: 3, nombre: "Estudiante de Prueba", correo: "estudiante@itsr.edu.mx", password: "estudiante123", rol: "estudiante" }
];
...
function getUsuarios() {
    ...
    if (stored) {
        ...
        // Asegurar que los usuarios predeterminados siempre estén presentes
        const missingDefaults = defaultUsuarios.filter(defaultUser => !usuarios.some(u => u.correo === defaultUser.correo));
        if (missingDefaults.length > 0) {
            const updatedUsuarios = [...usuarios, ...missingDefaults];
            localStorage.setItem('labtrack_usuarios', JSON.stringify(updatedUsuarios));
...
function getEquipos() {
    ...
    if (stored) {
        const equipos = JSON.parse(stored).map(equipo => ({
            ...equipo,
            estado: normalizeEquipoEstado(equipo.estado, equipo.stock)
        }));
...
function normalizeEquipoEstado(estado, stock = 0) {
    const normalized = (estado || '').toString().trim().toLowerCase();
    if (normalized === 'disponible' || normalized === 'available') return 'disponible';
    if (normalized === 'no_disponible' || normalized === 'no disponible' || normalized === 'agotado' || normalized === 'sin stock') return 'no_disponible';
...
function getPrestamos() {
    ...
    if (stored) {
        const prestamos = JSON.parse(stored);
        return updateOverduePrestamos(prestamos);
    }
    return [];
}
...
function updateOverduePrestamos(prestamos) {
    const today = new Date().toISOString().split('T')[0];
    let changed = false;

    prestamos.forEach(prestamo => {
        if (prestamo.estado === 'activo' && prestamo.fechaDevolucion && prestamo.fechaDevolucion < today) {
...
// Renderiza el menú lateral dinámicamente según el rol del usuario
function renderSidebarMenu(currentUser) {
    ...
    if (currentUser?.rol === 'admin') {
        sidebarMenu.innerHTML = `
            <a href="dashboard.html" class="menu-item">
                <i class="fas fa-tachometer-alt menu-item-icon"></i>
            ...
            <a href="prestamos.html" class="menu-item">
                <i class="fas fa-hand-holding menu-item-icon"></i>
                <span class="menu-item-text">Préstamos</span>
            </a>
            <a href="mantenimiento.html" class="menu-item">
                <i class="fas fa-tools menu-item-icon"></i>
                <span class="menu-item-text">Mantenimiento</span>
            </a>
            <a href="catalogo.html" class="menu-item">
                <i class="fas fa-store menu-item-icon"></i>
                <span class="menu-item-text">Ver Catálogo</span>
            </a>
            <a href="configuracion-admin.html" class="menu-item">
                <i class="fas fa-cog menu-item-icon"></i>
                <span class="menu-item-text">Configuración</span>
            </a>
            <a href="#" class="menu-item" onclick="logout(); return false;" style="margin-top: 50px; color: var(--danger);">
                <i class="fas fa-sign-out-alt menu-item-icon"></i>
                <span class="menu-item-text">Cerrar Sesión</span>
            </a>
        `;
        const avatarIcon = document.querySelector('.sidebar-user .user-avatar i');
        if (avatarIcon) avatarIcon.className = 'fas fa-user-cog';
    } else {
        sidebarMenu.innerHTML = `
            <a href="catalogo.html" class="menu-item active">
                <i class="fas fa-store menu-item-icon"></i>
                <span class="menu-item-text">Catálogo</span>
            </a>
            <a href="mis-prestamos.html" class="menu-item">
                <i class="fas fa-hand-holding menu-item-icon"></i>
                <span class="menu-item-text">Mis Préstamos</span>
            </a>
            <a href="configuracion-estudiante.html" class="menu-item">
                <i class="fas fa-cog menu-item-icon"></i>
...
function initApp() {
    ...
    // Actualizar menú lateral según el usuario (si existe)
...
function initCommonComponents() {
    ...
    (function() {
        ...
        if (!menuToggle) {
            ...
            if (headerContent) {
                headerContent.insertBefore(menuToggle, headerContent.children[1] || null);
            } else {
        ...
        document.addEventListener('click', (ev) => {
            if (!menuToggle.contains(ev.target)) {
                if (nav && nav.classList.contains('open') && !nav.contains(ev.target)) {
                    nav.classList.remove('open');
                    menuToggle.classList.remove('open');
                }
                if (sidebar && sidebar.classList.contains('active') && !sidebar.contains(ev.target)) {
...
function filterInventory() {
    ...
    const estado = document.getElementById('estadoFilter')?.value?.toLowerCase() || 'all';
    ...
    cards.forEach(card => {
        ...
        let cardEstado = (card.dataset.estado || card.querySelector('.stock-badge')?.textContent || '').toString().toLowerCase().trim();

        if (cardEstado === 'no disponible' || cardEstado === 'agotado' || cardEstado === 'sin stock') {
            cardEstado = 'no_disponible';
        } else if (cardEstado === 'disponible') {
            cardEstado = 'disponible';
        }
...
function initCatalogoPage() {
    ...
    // Actualizar la navegación del sidebar según el rol
    const sidebarMenu = document.getElementById('sidebarMenu');
    if (sidebarMenu && currentUser.rol === 'admin') {
        sidebarMenu.innerHTML = `
            <a href="dashboard.html" class="menu-item">
                <i class="fas fa-tachometer-alt menu-item-icon"></i>
                <span class="menu-item-text">Dashboard</span>
            ...
            <a href="login.html" class="menu-item" style="margin-top: 50px; color: var(--danger);">
                <i class="fas fa-sign-out-alt menu-item-icon"></i>
                <span class="menu-item-text">Cerrar Sesión</span>
            </a>
        `;
    ...
    const userEmailEl = document.querySelector('.sidebar-user .user-info p');
    if (userNameEl) userNameEl.textContent = currentUser.nombre;
    if (userEmailEl) userEmailEl.textContent = currentUser.matricula || currentUser.correo;
...
function requestLoan(equipmentId) {
    const currentUser = getCurrentUser();
    if (!currentUser || (currentUser.rol !== 'estudiante' && currentUser.rol !== 'admin')) {
        window.location.href = 'login.html';
        return;
    }
